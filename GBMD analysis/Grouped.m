% mastersheet for using Grouped sPCA to analyze GBMD affy data
...
% call Grouped sparse PCA to analyze GBMD_affy_processed_data
...
method='Grouped';    % method is Grouped sPCA
    
% Use BIC to select the best tuning parameter with training data
cumuvar='F';    % during cross-validation, do not generate the cumulative 
                % variation explained results
...
Groupedresults.firstloading=sum(rPCresults.rPCload(:,1)~=0);
Groupedresults.secondloading=sum(rPCresults.rPCload(:,2)~=0);

% check and save the cumulative proportions of variation explained in
% testing data set
Groupedresults.cumuvarprop=rPCresults.cumuvarprop;
...
Groupedresults.traincumuvarprop=cumuvarprop;

save './GBMD analysis/results/GBMD_affy_Groupedresults.mat';


%%%%%%%%%  This is extra step to present results!  %%%%%%%%%%

% plot the tuning parameter versus BIC values
plot(1:50,out.check(:,2))
saveas(gcf,'./GBMD analysis/results/Grouped_BIC_plot.jpeg')

% save PC loadings to a separate file
csvwrite('./GBMD analysis/results/Grouped_PCloadings.txt',rPCresults.rPCload);

gscatter(rPCresults.rPCload(:,1),rPCresults.rPCload(:,2))

% plot all subjects using the first two PCs grouped by their subtypes
twoPC=Xuse*rPCresults.rPCload;
twoPC(:,3)=SubtypeRec(:,2);
csvwrite('./GBMD analysis/results/Grouped_TwoPCs.txt',twoPC);
